...
class Home extends eui.Component {
    ...
    public constructor() {
        ...
        mc1.addEventListener(egret.Event.COMPLETE, (e: egret.Event) => {
            // mc1.gotoAndPlay( "back",1);
            // egret.Tween.get(this.dog_gp,{loop:true}).to({x:580,y:640},3000).to({x:368,y:550},3000);
            // egret.Tween.get(this.dog_gp).to({x:580,y:640},3000).call(()=>{
            //     egret.Tween.removeTweens(this.dog_gp);
            //     mc1.gotoAndPlay( "2",1);
            //     egret.Tween.get(this.dog_gp).to({x:368,y:550},3000);
            // })
        }, this);
        mc1.x = 0;
        mc1.y = 0;
    ...
    public eventListen() {
        var tanBtns: any[] = [
            ...
            // 仓库,个人信息(头像),个人日志,关闭日志(不是panel,在homeSkin内 visible为false),关闭排行            
        ...
        // 去掉果园
        var yuanBtns: any[] = [ this.my_jiayuan];
    ...
    public btnTapHandler(evt: egret.TouchEvent): void {
        // 打开 关闭    面板  一对一对一   todo
        var openBtns: any[] = [this.rizhi, this.paihang, this.chongzhi, this.open_save_house, 
            this.open_user_msg, this.open_shop, this.open_duihuan, this.open_zhuangban,
             this.tishi_left, this.tishi_right, this.up_level, this.up_house];
        var closeBtns: any[] = [this.close_rizhi, this.close_paihang, this.close_chongzhi, 
        this.close_save_house, this.close_user_msg, this.close_shop, this.close_duihuan, 
        this.close_zhuangban, this.close_land_info, this.close_bozhong, this.close_build_center, , this.close_build_center];
        var panel: any[] = [this.day_record, this.rank_gp, this.more_money, this.save_house, this.user_msg,
         this.shop, this.duihuan, this.zhuangban, this.land_info, this.bozhong, this.build_center, this.build_center];
    ...
    public creatRankList() {
        ...
        // 游戏商店
        var reXiaoArr: Array<Object> = [{ shopnum: "500", money: 50, left: 100000, info: '小麦' }];
